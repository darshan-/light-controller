...
func KbdIsStarted(timeout time.Duration) bool {
	select {
	case ping <- true:
		return true
...
func KbdGetKeys(bufferSize int) (<-chan KeyEvent, error) {
	if KbdIsStarted(time.Millisecond * 1) {
...
func KbdGetKey() (rune, Key, error) {
	// Check if opened
	if !KbdIsStarted(time.Millisecond * 50) {
...
func KbdGetSingleKey() (ch rune, key Key, err error) {
	err = KbdOpen()
	if err == nil {
		ch, key, err = KbdGetKey()
		errClosing := KbdClose()
		if err == nil {
			err = errClosing
		}
